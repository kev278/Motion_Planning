...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x1 = (self.start.row + self.goal.row) / 2
            x2 = (self.start.col + self.goal.col) / 2
            x_center = [x1, x2]
            # Compute rotation matrix from elipse to world frame - C
            col = self.goal.col - self.start.col
            row = self.goal.row - self.start.row
            theta = math.atan2(col, row)
            C = [[np.cos(theta), -np.sin(theta)],
                 [np.sin(theta),  np.cos(theta)]]
            # Compute diagonal matrix - L
            elem1 = c_best / 2
            elem2 = np.sqrt(c_best**2 - c_min**2) / 2
            mat = [elem1, elem2]
            L = np.diag(mat)
            ...
            var = np.dot(C, L)
            x_rand = np.dot(var, x_ball) + x_center
    ...
    def informed_RRT_star(self, n_pts, neighbor_size=20):
        ...
        for i in range(n_pts):
            ...
            if self.found:
                #if self.path_cost(self.start, self.goal) < c_best:
