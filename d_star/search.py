...
class DStar:
    ...
    def process_state(self):
        ...
        min_elem = self.min_node()
        k_min = self.get_k_min()
        self.delete(min_elem)
        # Get neighbors of the node
        # using self.get_neighbors
        nbh = self.get_neighbors(min_elem)
        # If node k is smaller than h (RAISE)
        if k_min < min_elem.h:
            for i in nbh:
                if i.h < k_min and min_elem.h > i.h + self.cost(i, min_elem):
                    min_elem.parent = i
                    min_elem.h = i.h + self.cost(i, min_elem)
        # If node k is the same as h (LOWER)
        if k_min == min_elem.h:
            for i in nbh:
                if i.tag == "NEW" or (i.parent == min_elem and i.h != (min_elem.h + self.cost(i, min_elem))) or (i.parent != min_elem and i.h > (min_elem.h + self.cost(i, min_elem))):
                    i.parent = min_elem
                    self.insert(i, min_elem.h + self.cost(i, min_elem))
        # Else node k is smaller than h (RASIE)
        else:
            for i in nbh:
                if i.tag == "NEW" or (i.parent == min_elem and (i.h != min_elem.h + self.cost(i, min_elem))):
                    i.parent = min_elem
                    self.insert(i, min_elem.h + self.cost(i, min_elem))
                    
                else:
                    if i.parent != min_elem and i.h > min_elem.h + self.cost(i, min_elem):
                        self.insert(min_elem, min_elem.h)
                    else:
                        if i.parent != min_elem and min_elem.h > i.h + self.cost(i, min_elem) and i.tag == "CLOSED" and i.h > k_min:
                            self.insert(i, i.h)
        #### TODO END ####
    ...
    def repair_replan(self, node):
        ...
        while not(k_min >= node.h or k_min == -1):
            k_min = self.process_state()
    ...
    def prepare_repair(self, node):
        ...
        nbh = self.get_neighbors(node)
        # If neighbor.is_dy_obs == True but neighbor.is_obs == Flase, 
        # the neighbor is a new dynamic obstacle
        for i in nbh:
            if not i.is_obs and i.is_dy_obs:
                n_nbh = self.get_neighbors(node)
                for j in n_nbh:
                    self.modify_cost(i, j)
    ...
    def run(self):
        ...
        while(self.start.tag != "CLOSED" and len(self.open) != 0):
            k_min = self.process_state()
